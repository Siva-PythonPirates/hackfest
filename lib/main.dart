...
import 'package:hackfest_demo/login.dart';
...
void main() async {
  ...
  await Firebase.initializeApp(
    options: const FirebaseOptions(
      apiKey: "AIzaSyBS-3lYpMmUQEZyu9ZH_euxGfMEZhVysj0",
      appId: "1:619674404650:android:e005d4f9d9f08ab52b8053",
      messagingSenderId: "619674404650",
      projectId: "hackfest-demo-1",
    ),
  );
  runApp(MyApp());
}
...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        '/': (context) => LoginPage(),
        '/admin': (context) => AdminDashboard(alertMessages: [],),
      },
    );
  }
}
...
class _MyBodyState extends State<Body> {
  ...
  Future<void> _sendAlertMessage() async {
    ...
    String alertMessage = "Alert: Device at (${currentLocation.latitude}, ${currentLocation.longitude})";

    endpointMap.forEach((key, value) {
      Nearby().sendBytesPayload(key, Uint8List.fromList(alertMessage.codeUnits))
          .then((_) {
        showSnackbar("Alert message sent to ${value.endpointName}");
      }).catchError((error) {
        showSnackbar("Error sending alert message to ${value.endpointName}: $error");
      });
    });
  }
